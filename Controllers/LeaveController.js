...
module.exports = {
  addLeave: async (req, res) => {
    try {
      ...
      if (
        leave_for == "FIRST_HALF" ||
        (leave_for == "SECOND_HALF" && leave_from != leave_to)
      ) {
        return res.status(401).json({
          status: false,
          message: "Start Date And End Date Should Be Same For Half Day Leave",
        });
      }
  ...
  getLeaveByEmployeeId: async (req, res) => {
    try {
      ...
      if (leave.length == 0) {
        return res.status(404).json({
          status: false,
          message: `Leave Not Found With User ID :- ${employee_id} `,
        });
      }
      return res
        .status(200)
  ...
  getLeaveByStatus: async (req, res) => {
    try {
      ...
      if (leave.length == 0) {
        return res.status(404).json({
          status: false,
          message: `Leave Not Found With Status :- ${status} `,
        });
      }
      return res
        .status(200)
  ...
  getLeaveByDate: async (req, res) => {
    try {
      ...
      const leave = await leaveModel.find({
        $and: [
          { leave_from: { $gte: new Date(start_date) } },
          { leave_to: { $lte: new Date(end_date) } },
        ],
      });
      if (leave.length == 0) {
        return res.status(404).json({
          status: false,
          message: `Leave Not Found Between :- ${start_date} - ${end_date} `,
        });
      }
      return res
        .status(200)
  ...
  deleteLeave: async (req, res) => {
    try {
      ...
      return res.status(200).json({
        status: true,
        message: `Leave Deleted Successfully with ID :- ${leave._id}`,
      });
    } catch (err) {
      return res.status(500).json({
        status: false,
